window.onload = function () {
  ...
  form.addEventListener("submit", function (event) {
    ...
    listCake.forEach((list) => {
      if (list.type.toLocaleLowerCase.includes(findCake.value)) {
        show.innerHTML = "";
        var daTimRa = listCake.filter((listCake) =>
          listCake.type.toLocaleLowerCase.includes(findCake.value)
        );
        console.log(daTimRa);
      ...
      if (list.type.includes(findCake.value) == false) {
        show.innerHTML = "";
  ...
  function findKey() {
    listCake.forEach((list) => {
      if (list.type.toLowerCase.includes(findCake.value)) {
        show.innerHTML = "";
        var daTimRa = listCake.filter((l) =>
          list.type.toLowerCase.includes(findCake.value)
        );
        daTimRa.forEach((list) => {
          var col = document.createElement("div");
          col.classList.add("col-4", "mb-3");
